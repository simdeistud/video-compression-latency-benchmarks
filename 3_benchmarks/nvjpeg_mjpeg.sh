...
# Usage: ./nvjpeg_to_lossless.sh <subsampling:{420,422,444}> <quality:0-100> <input_video> <output.{mkv|zip}>
# Example: ./nvjpeg_to_lossless.sh 420 75 input.mp4 out.mkv
#          ./nvjpeg_to_lossless.sh 444 90 input.y4m out.zip
...
NVJPEG="/home/simone/Documenti/video-compression-latency-benchmarks/3_benchmarks/src/nvjpeg/cmake-build-debug/nvjpeg_encode"
...
JPEG_DIR="$WORKDIR/jpeg"  # per-frame JPEG from nvjpeg
...
echo "[2/3] PPM -> raw rgb24 -> nvjpeg_encode (-> per-frame JPEG)..."

# Encode each frame with your nvjpeg (stdin: raw rgb24, stdout: JPEG -> file)
...
for f in "$PPM_DIR"/frame_*.ppm; do
  ...
  # PPM -> raw rgb24 to stdout, then pipe to nvjpeg
...
echo "  - $JPEG_DIR (per-frame JPEG from nvjpeg)"
