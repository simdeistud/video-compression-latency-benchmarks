// Single-frame AV1 (libaom-av1) encode micro-benchmark.
// Encodes the SAME raw frame 'iterations' times, measures encode time only.
//
// Usage:
// av1_encode_frame <raw_frame.yuv> <WxH> <yuv420p|yuv422p|yuv444p> <cpu_mode(0|1)>
...
// - Encoder configured for all-intra (GOP=1), low-latency (no lookahead).
...
static int av1_profile_for_pixfmt(enum AVPixelFormat pf) {
  // AV1: profile 0 = 4:2:0; profile 1 = 4:2:2 / 4:4:4 (8-bit paths here).
  switch (pf) {
...
static int open_av1_ctx(AVCodecContext **pctx,
                        ...
                        int use_all_cpus, RcMode rc_mode, int rc_value)
{
  ...
  if (!codec) { fprintf(stderr, "libaom-av1 not available.\n"); return -1; }

  AVCodecContext *ctx = avcodec_alloc_context3(codec);
  if (!ctx) { fprintf(stderr, "Cannot alloc codec ctx.\n"); return -1; }

  // Base config: all-intra, low-latency, multi-threaded
  ...
  // Libaom private options
  av_opt_set_int(ctx->priv_data, "cpu-used", use_all_cpus ? 8 : 2, 0); // speed/quality knob
  av_opt_set_int(ctx->priv_data, "lag-in-frames", 0, 0);               // no lookahead
  av_opt_set_int(ctx->priv_data, "row-mt", 1, 0);                      // row-based MT
  // Tiling to scale threads (log2). Example: 4 cols, 2 rows when using all cores.
  if (use_all_cpus) {
  ...
  // Profile to match subsampling
  av_opt_set_int(ctx->priv_data, "profile", av1_profile_for_pixfmt(pf), 0);
  // Disable alt-ref for this intra/low-latency micro-benchmark
  av_opt_set_int(ctx->priv_data, "auto-alt-ref", 0, 0);

  // Rate control
  if (rc_mode == RC_CRF) {
    // CRF 1..63 (0=lossless). Pure CQ if no bitrate caps are set.
  ...
  if (avcodec_open2(ctx, codec, NULL) < 0) {
    ...
    fprintf(stderr, "Failed to open libaom-av1.\n");
    return -1;
  }
  *pctx = ctx;
  return 0;
...
int main(int argc, char **argv) {
  ...
  if (argc < 8 || argc > 9) {
    fprintf(stderr,
      ...
      "  <rc_mode: crf|cbr|cbr_strict> <rc_value> <iterations> [output -]\n"
      "Notes:\n"
      " - crf: rc_value = CRF in 0..63 (0=lossless; typical 1..63)\n"
  ...
  if (pix_fmt == AV_PIX_FMT_NONE) {
    fprintf(stderr, "Unsupported subsampling. Use yuv420p|yuv422p|yuv444p (8-bit raw).\n");
    return 1;
  }
  ...
  if (rc_mode == RC_CRF) {
    if (rc_value < 0 || rc_value > 63) { fprintf(stderr, "CRF must be 0..63 (AV1).\n"); return 1; }
  } else {
  ...
  if (open_av1_ctx(&ctx, width, height, pix_fmt, use_all_cpus, rc_mode, rc_value) < 0) {
