...
RESTART_INTERVALS = [0, 1, 2, 4, 8, 16, 32, 64, 128] #range(0, 9, 2)
...
#%%
results = []
total_combinations = len(RESOLUTIONS) * len(QUALITIES) * len(FORMATS) * len(SUBSAMPLINGS) * len(RESTART_INTERVALS)
...
df.to_csv('results_gpujpeg_complete.csv', index=False)
print("\n✅ Benchmarking complete! Results saved to 'results_gpujpeg_complete.csv'")
df.head()
