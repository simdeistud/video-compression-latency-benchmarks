...
int main(int argc, char** argv)
{
    ...
    CHECK_CUDA(cudaHostAlloc(reinterpret_cast<void**>(&pinned_inbuf), inbuf_size, cudaHostAllocDefault));
    memcpy(pinned_inbuf, inbuf, inbuf_size);

    nvjpegImage_t src;
    src.channel[0] = pinned_inbuf;
    src.pitch[0] = pitch;
