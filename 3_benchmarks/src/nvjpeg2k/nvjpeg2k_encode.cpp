...
int main(int argc, char **argv)
{
    ...
    CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&d_rgb), rgb_size));
    ...
    for (int it = 0; it < total_iters; ++it) {
        ...
        CHECK_CUDA(cudaMemcpyAsync(
            d_rgb, host_rgb.data(), rgb_size, cudaMemcpyHostToDevice, stream));
        ...
        last_bitstream.swap(bitstream);
    }

    if (opt.benchmark) {
        if (opt.output_path != "-") {
            write_output(opt.output_path, last_bitstream.data(), last_bitstream.size());
        }
        std::printf("%.9f\n", total_s);
    } else {
        write_output(opt.output_path, last_bitstream.data(), last_bitstream.size());
