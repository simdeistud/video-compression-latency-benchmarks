...
# Usage: ./gpujpeg_to_lossless.sh <subsampling:{420,422,444}> <quality:0-100> <input_video> <output.{mkv|zip}>
# Example: ./gpujpeg_to_lossless.sh 420 75 input.mp4 out.mkv
#          ./gpujpeg_to_lossless.sh 444 90 input.y4m out.zip
...
gpujpeg="/home/simone/Documenti/video-compression-latency-benchmarks/3_benchmarks/src/gpujpeg/cmake-build-debug/gpujpeg_encode"
...
JPEG_DIR="$WORKDIR/jpeg"  # per-frame JPEG from gpujpeg
...
echo "[2/3] PPM -> raw rgb24 -> gpujpeg_encode (-> per-frame JPEG)..."

# Encode each frame with your gpujpeg (stdin: raw rgb24, stdout: JPEG -> file)
...
for f in "$PPM_DIR"/frame_*.ppm; do
  ...
  # PPM -> raw rgb24 to stdout, then pipe to gpujpeg
...
echo "  - $JPEG_DIR (per-frame JPEG from gpujpeg)"
